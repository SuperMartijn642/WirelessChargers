...
public class ChargerRenderer implements CustomBlockEntityRenderer<ChargerBlockEntity,ChargerRenderer.State> {
    ...
    public void updateState(State state, ChargerBlockEntity entity, UpdateContext context){
        ...
        List<BlockStateModelPart> parts = state.ringRenderState.setupModel(IDENTITY_MATRIX, ringModel.hasMaterialFlag(level, pos, blockState, BakedQuad.FLAG_TRANSLUCENT));
        ringModel.collectParts(level, pos, blockState, random, parts);
