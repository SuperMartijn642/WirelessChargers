...
public class ChargerRenderer implements CustomBlockEntityRenderer<ChargerBlockEntity,ChargerRenderer.State> {
    ...
    public void updateState(State state, ChargerBlockEntity entity, UpdateContext context){
        ...
        QuadEmitter emitter = state.ringRenderState.setupMesh(IDENTITY_MATRIX, ringModel.hasMaterialFlag(level, pos, blockState, random, BakedQuad.FLAG_TRANSLUCENT));
        ringModel.emitQuads(emitter, level, pos, blockState, random, _ -> false);
